...
### Changed — 2026-04-19 (install path)
- **`onnxruntime` is no longer a main dependency. Pick `[cpu]` or `[gpu]`.** `onnxruntime` (CPU) and `onnxruntime-gpu` share the same Python import path and silently collide when both are installed — the CPU one wins and `CUDAExecutionProvider` never registers. Previously `pip install -e '.[gpu]'` dragged CPU `onnxruntime` back in via the main deps, producing a broken dual install every time. The ONNX Runtime is now split between two mutually-exclusive extras: `[cpu]` (new) and `[gpu]` (existing). If neither is installed, `FaceDetector` raises `MissingOnnxRuntimeError` — caught in `cli/app.py`'s `main()` — and the CLI exits 6 with the exact install commands. Migration: `pip install -e '.[cpu]'` (or `'.[gpu]'`) on fresh installs; existing dual installs should `pip uninstall -y onnxruntime onnxruntime-gpu` and reinstall exactly one.
